...
# Docker Deploy Script for Fake Google
...
show_help() {
    ...
    echo "  -b, --build-only        Only build images, don't start containers"
    echo "  -c, --clean             Clean build (no cache)"
    echo "  -l, --logs              Show logs after starting"
    echo "  -s, --stop              Stop containers only"
    echo ""
    echo "Examples:"
    echo "  ./docker-deploy.sh                # Production deployment"
    echo "  ./docker-deploy.sh --dev          # Development deployment"
    echo "  ./docker-deploy.sh --clean        # Clean production build"
    echo "  ./docker-deploy.sh --stop         # Stop all containers"
    echo "  ./docker-deploy.sh --logs         # Start and show logs"
}
